...
#define TDCP_WEIGHT (double) 50.0
...
int main(int argc, char** argv) {
    ...
    for (int v =0; v < ver_num; v++) {
        ...
        cout << "=========================== Version " << v+1 <<" Starts ==========================="<< endl;
        std::string folder_name = matlab_save_dir + "/monte_carlo"+tau_str+"/epoch_" + std::to_string(start_epoch + 1) + "_T_" + std::to_string(T);
        std::string version = "/v" + std::to_string(v+1);  // version 1 to 100
        std::string rover_dir = "../data/monte_carlo" +tau_str + "/data_rover" + version;
        std::string station_dir = "../data/monte_carlo" +tau_str + "/data_base" + version;
            ...
            if (covariance_result){
                ...
                for (int i = 0; i < val_num; i++) {
                    for (int j = 0; j < val_num; j++) {
                        std::cout << covariance_matrix[val_num * i + j] << " ";
                        fout_cov << covariance_matrix[val_num * i + j] << ", ";
                    }
                    std::cout << "\n";
                    fout_cov << "\n";
                }
